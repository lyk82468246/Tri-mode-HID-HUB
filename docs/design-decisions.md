...
PS/2 插座按常见 5-pin 逻辑假设：数据、时钟、5 V、GND，另保留的机械/未用针脚均标 NC。键盘和鼠标各用两颗 BSS138 构成四路双向电平转换，低压侧上拉到 3V3，高压侧上拉到 5V_HOST。

这一部分必须用最终采购的 PS/2 插座数据手册核对 pin 1/3/4/5；不能仅依据封装库名称。
